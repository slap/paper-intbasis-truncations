...
\item By Proposition \ref{prop:factors} we can compute the factors in $K[[X]][Y]$ correct up to degree $n = K - \IntExpY \ge \alpha + \IntExpY$. Consider the $i$-th branch. $n - \tilde v_i \ge n - \IntExpY = K - 2 \IntExpY \ge \alpha \ge r_i$. Then by Algorithm \ref{algo:halfRNP}, we can compute the $Y$-Puiseux expansions of this branch correct up to degree $(n - \tilde v_i) / e_i$. Let $D = n - \tilde v_i$.
\item Assume that the Puiseux expansions in $X$ of the $i$-th branch have denominator $\epsilon_i$ (the Puiseux expansions in $Y$ have denominator $e_i$). Then the polynomial $\eta(U)$ corresponding to that branch is correct up to degree $D$: the Puiseux expansion $\eta(Y^{1/m})$ is correct to degree $D/m$ and we take $U = Y^{1/m}$.
\item Applying the inverse formula we get a polynomial $\gamma(T)$ which is correct up to the same degree $D$ in $T$. Now we replace $T = X^{1/\epsilon_i}$ to get the Puiseux expansions in $X$, $\gamma(X^{1/\epsilon_i})$. We get correct Puiseux expansions up to degree $D/\epsilon_i$.
\item We need this number $D/\epsilon_i$ to be bigger than $\IntExpX$. So we get $D \ge \epsilon_i\IntExpX$ and then $K - \IntExpY - \tilde v_i \ge \epsilon_i \IntExpY$. So we get $K \ge \epsilon_i\IntExpX + \IntExpY + \tilde v_i$.

\item Consider $\epsilon$ the largest $\epsilon_i$ over all branches. So we need
$$K > \epsilon \IntExpX + \IntExpY + ,$$
