...
\item I am not sure about this last step. In any case, we can compute the singular part of the Puiseux expansions by truncating at $\alpha + 2 \times \IntExpX$ (a little larger than $3 \times \IntExpX$). The argument is as follows:
    ...
    \item So by the algorithm we need $n \ge v_i + r_i$, and $v_i + r_i \le \alpha + \IntExpX$. That is, we need the factor $F_i$ developed to degree $\alpha + \IntExpX$ and by Proposition \ref{prop:factors} we can get this by computing a factorization of $F$ modulo $\alpha + 2 \IntExpX$.
    \end{enumerate}
...
\item By Proposition \ref{prop:factors} we can compute the factors in $K[[X]][Y]$ correct up to degree $n = K - \IntExpY \ge \alpha + \IntExpY$. Consider the $i$-th branch. $n - v_i \ge n - \IntExpY = K - 2 \IntExpY \ge \alpha \ge r_i$. Then by Algorithm \ref{algo:HalfRNP}, we can compute the $Y$-Puiseux expansions of this branch correct up to degree $(n - v_i) / e_i$. Let $D = n - v_i$.
...
\item Applying the inverse formula we get a polynomial $\gamma(T)$ which is correct up to the same degree $D$ in $T$. Now we replace $T = X^{1/n}$ to get the Puiseux expansions in $X$, $\gamma(X^{1/n})$. We get correct Puiseux expansions up to degree $D/n$.
\item We need this number $D/\epsilon_i$ to be bigger than $\IntExpX$. So we get $D \ge \epsilon_i\IntExpX$ and then $K - \IntExpY - v_i \ge n \IntExpY$. So we get $K \ge \epsilon_i\IntExpX + \IntExpY + v_i$.
