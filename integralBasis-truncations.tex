...
$$G_k \equiv G_k^* \mod Y^{N_0 - \kappa}$$
where $G_k^*$ are the analytic factors of $F$ in $K[[Y]][X]$.

By the same argument $G_k$ agrees with the analytic factors of $\overline{F}^{Y} \mod Y^{N_0 - \kappa}$.
Hence: the analytic factors in $K[[Y]][X]$ of $F$ and $\overline{F}^{Y}$ are the same modulo $Y^{N_0 - \kappa}$.
...
\begin{enumerate}
\item Let $\bar F$ be the truncation of $F$ at some degree $K$ in $Y$. We want to know a good $K$.
\item Assume $K \ge 3 \IntExpY$. 
\item By Proposition \ref{prop:factors}, since $K \ge 2 \IntExpY$, we can compute the factors in $K[[X]][Y]$ correct up to degree $n = K - \IntExpY$.
    
\item Consider the $k$-th branch. We have
$$n - \tilde v_k \ge n - \IntExpY = K - 2 \IntExpY \ge \IntExpY \ge \tilde v_k \ge \frac{r_k}{\epsilon_k}.$$

Then by Lemma \ref{lemma:pebound}, since the factor is computed with truncation bound $n = K - \IntExpY$, we obtain the Puiseux expansions with precision $n_0 = n - \tilde v_k \ge \IntExpY$. Let $D = n - \tilde v_k$.
\item Assume that the Puiseux expansions in $Y$ of the $k$-th branch of $F$ have denominator $\epsilon_k$ ((the Puiseux expansions in $X$ have denominator $e_k$). Then the polynomial $\eta(U)$ corresponding to that branch is correct up to degree $D\epsilon_k$: the Puiseux expansion $\eta(Y^{1/{\epsilon_k}})$ is correct to degree $D$ and we take $U = Y^{1/{\epsilon_k}}$.
\item Applying the inverse formula we get a polynomial $\gamma(T)$ which is correct up to the same degree $D\epsilon_k$ in $T$. Now we replace $T = X^{1/e_k}$ to get the Puiseux expansions in $X$, $\gamma(X^{1/e_k})$. We get correct Puiseux expansions up to degree $D\epsilon_k/e_k$.
\item We need this number $D \epsilon_k / e_k$ to be bigger than $\IntExpX$. So we get $D \ge \frac{e_k}{\epsilon_k}\IntExpX$ and then $K - \IntExpY - \tilde v_k \ge \frac{e_k}{\epsilon_k} \IntExpY$. So we get $K \ge \frac{e_k}{\epsilon_k}\IntExpX + \IntExpY + \tilde v_k$.

\item Consider $\frac{e}{\epsilon}$ the largest $\frac{e_k}{\epsilon_k}$ over all branches. So we need
$$K > \frac{e}{\epsilon} \IntExpX + 2 \IntExpY,$$
...
\begin{remark}
The argument for truncations in $Y$ is the following:
\begin{enumerate}
\item Let $\bar F$ be the truncation of $F$ at some degree $K \ge \alpha_Y + 2 \IntExpY$ in $Y$. We want to know a good $K$.
\item By Proposition \ref{prop:factors} we can compute the factors in $K[[X]][Y]$ correct up to degree $n = K - \IntExpY \ge \alpha + \IntExpY$. Consider the $i$-th branch. $n - \tilde v_i \ge n - \IntExpY = K - 2 \IntExpY \ge \alpha \ge r_i$. Then by Algorithm \ref{algo:halfRNP}, we can compute the $Y$-Puiseux expansions of this branch correct up to degree $n - \tilde v_i$. Let $D = n - \tilde v_i$.
\item Assume that the Puiseux expansions in $X$ of the $i$-th branch have denominator $\epsilon_i$ (the Puiseux expansions in $Y$ have denominator $e_i$). Then the polynomial $\eta(U)$ corresponding to that branch is correct up to degree $De_i$: the Puiseux expansion $\eta(Y^{1/{e_i}})$ is correct to degree $D$ and we take $U = Y^{1/{e_i}}$.
\item Applying the inverse formula we get a polynomial $\gamma(T)$ which is correct up to the same degree $De_i$ in $T$. Now we replace $T = X^{1/\epsilon_i}$ to get the Puiseux expansions in $X$, $\gamma(X^{1/\epsilon_i})$. We get correct Puiseux expansions up to degree $De_i/\epsilon_i$.
\item We need this number $De_i/\epsilon_i$ to be bigger than $\IntExpX$. So we get $D \ge \frac{\epsilon_i}{e_i}\IntExpX$ and then $K - \IntExpY - \tilde v_i \ge \frac{\epsilon_i}{e_i} \IntExpY$. So we get $K \ge \frac{\epsilon_i}{e_i}\IntExpX + \IntExpY + \tilde v_i$.

\item Consider $\frac{\epsilon}{e}$ the largest $\frac{\epsilon_i}{e_i}$ over all branches. So we need
$$K > \frac{\epsilon}{e} \IntExpX + 2 \IntExpY,$$
which is the bound we were looking for.

\end{enumerate}

%\textbf{Note:} Going through Puiseux expansions it seems that we get a large degree bound.
%It sounds plausible to get a condition $D e_i / \epsilon_i \ge \IntExpX$ instead of the condition above, and in this case we would get a bound
%$$K > \epsilon/e \IntExpX + \IntExpY + v,$$
%where $\epsilon/e$ is the largest value among all $\epsilon_i / e_i$ and $v$ is the largest $v_i$.
%
%This sounds like a more reasonable bound. It seems that we need to invert the factors without going through Puiseux expansions to get a bound like this one.
