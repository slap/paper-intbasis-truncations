...
So let
$$(x(U), y(U)) = (a_0 U^{m_0} + a_1 U^{m_0+1} + a_2 U^{m_0+2} + \dots, U^n),$$
$a_0 \neq 0$, be a parametrization of the $k$-th branch (here $n = e_k$ and $m_0 = d_k$). The Puiseux expansions are 
$$
\eta_j = a_0 (\zeta u)^{\frac{m_0}{n}} +  a_1 (\zeta u)^{\frac{m_0 + 1}{n}} +  a_2 (\zeta u)^{\frac{m_0 + 2}{n}}+ \dots,
$$ 
where $\psi$ is an $n$-th root of unity and
$$
...
\begin{align*}
(x(U), y(U)) &= (U^{m_0} (a_0 + a_1 U + a_1 U^2 + \dots), U^n) \\
             &= \left(\left(U \sqrt[m_0]{a_0 + a_1 U + a_2 U^2 + \dots}\right)^{m_0}, U^n\right)
\end{align*}
...
$$
be an $m_0$-th root of $a_0 + a_1 U + a_2 U^2 + \dots$. In computations, if $a_0 + a_1 U + a_1 U^2 + \dots$ is developed up to precision $k$, then also $b_0 + b_1 U + b_2 U^2 + \dots$ can be computed with precision $k$.

Set $q(U) = U (b_0  + b_1 U + b_2 U^2 + \dots) = b_0 U + b_1 U^2 + b_2 U^3 + \dots$ (hence $x(U) = q(U)^{m_0}$). Again, $q(U)$ can be computed with precision $k$.

Finally let $p(T) = c_1 T + c_2 T^2 + \dots$ be the composite inverse of $q(U)$ (which can also be computed with precision $k$), that is $q(p(T)) = T$ and hence $U = p(T)$.

We obtain the new parametrization
$$(T^{m_0}, p(T)^n).$$

As we have seen, all these transformations can be done without precision loss, hence if the $x$-Puiseux expansions of $G^*_i(x,y)$ are know with precision $k/n$.
%
