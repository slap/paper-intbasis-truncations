...
\item $v_i \le \alpha$ when there is only one branch, but not in general.
\end{itemize}
...
\item I am not sure about this last step. In any case, we can compute the singular part of the Puiseux expansions by truncating at three times the integrality exponent. The argument is as follows:
    ...
    \item So by the algorithm we need $n \ge v_i + r_i$, and $v_i + r_i \le \alpha + \IntExpX$. That is, we need the factors $F_i$ developed to degree $\alpha + \IntExpX$, and we can get this by computing a factorization of $F$ modulo $\alpha + 2 \IntExpX$.
    \end{enumerate}

\item Putting everything together, we can truncate the powers of $X$ of $f$ at order two times the integrality exponent and we will be able to compute the correct factors and Puisuex expansions up to the integrality exponent and hence we will compute the correct integral basis.
...
\begin{enumerate}
\item Let $\bar F$ be the truncation of $F$ at some degree $K > 2 \IntExpY$ in $Y$. We want to know a good $K$.
\item Assume the $Y$-Puiseux expansions of the truncated polynomial are correct up to degree $D = K - \IntExpY$ (I wrote that we can deduce this from Proposition 23 but I am not sure how).
\item Consider a branch where the Puiseux expansions in $X$ have denominator $n$ and the Puiseux expansions in $Y$ have denominator $m$. Then the polynomial $\eta(U)$ corresponding to that branch is correct up to degree $Dm$ (the Puiseux expansion $\eta(Y^{1/m})$ is correct to degree $D$ and we take $U = Y^{1/m}$).
\item Applying the inverse formula we get a polynomial $\gamma(T)$ which is correct up to the same degree $Dm$ in $T$. Now we replace $T = X^{1/n}$ to get the Puiseux expansions in $X$, $\gamma(X^{1/n})$. We get correct Puiseux expansions up to degree $Dm/n$.
\item We need this number $Dm/n$ to be bigger than $\IntExpX$.
