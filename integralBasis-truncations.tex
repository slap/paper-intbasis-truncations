...
\begin{enumerate}
\item Let $\overline{F}^{Y}$ be the truncation of $F$ at some degree $K$ in $Y$. We want to know a good $K$.
\item Assume $K \ge 3 \IntExpY$.
\item We can factorize $\overline{F}^{Y}$ in $K[[Y]][X]$:
$$\overline{F}^{Y} \equiv G_0 G_1 \dots G_s \mod Y^{N_0}.$$

\item By Proposition \ref{prop:factors}, since $K \ge 2 \IntExpY$, the factors are correct up to degree $n = K - \IntExpY$.

\item Consider the $k$-th branch. Let $\epsilon_k$ be the ramification index of $G_k$ (the largest denominator in the expansions), $\tilde v_k = v_Y((G_K(\eta(Y^{1/\epsilon_k})))$ and $r_k$ the regularity index (so that the singular part goes up to degree $r_k/\epsilon_k$).
...
$$K - \IntExpY - \tilde v_k \ge \frac{e_k}{\epsilon_k} \IntExpY.$$
So we get 
$$K \ge \frac{e_k}{\epsilon_k}\IntExpX + \IntExpY + \tilde v_k.$$
