...
\begin{enumerate}
\item Let $\bar F$ be the truncation of $F$ at some degree $K > \alpha_Y + 2 \IntExpY$ in $Y$. We want to know a good $K$.
\item By Proposition \ref{prop:factors} we can compute the factors in $K[[X]][Y]$ correct up to degree $K - \IntExpY \ge \alpha + \IntExpY$, and hence we can compute the $Y$-Puiseux expansions of the truncated polynomial correct up to degree $D = (K - \IntExpY) / e_i$.
\item Consider a branch where the Puiseux expansions in $X$ have denominator $n$ and the Puiseux expansions in $Y$ have denominator $m =e_i$. Then the polynomial $\eta(U)$ corresponding to that branch is correct up to degree $D$ (the Puiseux expansion $\eta(Y^{1/m})$ is correct to degree $D/m$ and we take $U = Y^{1/m}$).
...
\item We need this number $D/n$ to be bigger than $\IntExpX$. Question: can we request only $D > \IntExpX$? (that is, if we know the expansions up to degree $D/n$ can we recover the factor to degree $D$? We assume it is possible. If not, we should take $1/n$ instead of $m/n$ in the following steps.
\item Consider the smaller $m/n$ over all branches. So we need $D m/n > \IntExpX$, hence
$$
m/n (K - \IntExpY) > \IntExpX.
$$
\item This gives
$$K > n/m \IntExpX + \IntExpY,$$
which is the bound we were looking for (this should be something similar to $2 \IntExpY$, we could do this computation, but the formula we got is good enough).
\end{enumerate}
