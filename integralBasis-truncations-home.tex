...
For computing Puiseux expansions we use the following algorithm:
...
The notation in the algorithm is as follows:
\begin{itemize}
\item $\delta = \upsilon_X \left( \operatorname{Res}_Y(F, F_Y) \right)$
\item We can consider $\mathbb{K}_P = \mathbb{Q}$, and $H$ is our polynomial $f$.
\item $v_i = v_X({(F_i)}_Y)$. If $f$ has only only one branch, then this number is the integrality exponent. If $f$ has many branches, then $v_i$ can be bigger than the integrality exponent (
\item The integer $e_i \in\mathbb{N}$ is the ramification index of $F_i$, equal to the largest denominator or the number of expansions in the branch.
\item The \emph{regularity index} \( r \) of a Puiseux series \( S \) of \( F \) with ramification index \( e \) is the least integer \( N \ge \min(0, e \upsilon_X(S)) \) such that, if
\[
\left\lceil S \right\rceil^{N/e} = \left\lceil S' \right\rceil^{N/e}
\]
for some Puiseux series \( S' \) of \( F \), then \( S = S' \). We call \( \left\lceil S \right\rceil^{r/e} \) the \emph{singular part} of \( S \) in \( F \).

\item We denote by \( (R_i)_{1 \le i \le \rho} \) the rational Puiseux expansions of \( F \). To any \( R_i \), we will always associate the following notations:
\begin{itemize}
    \item \( e_i \), \( f_i \), and \( r_i \) will respectively denote the ramification index, the residual degree, and the regularity index of \( R_i \),
    \item we define \( v_i \in \mathbb{Q} \) as \( \upsilon_X(F_Y(S)) \), for any Puiseux series \( S \) associated to \( R_i \).
\end{itemize}
The same notations will be used if \( S_i \) (or \( S_{ijk} \)) denotes a Puiseux series. If we omit the index \( i \), we will use the notations \( e \), \( f \), and \( r \) for the first three integers.

\end{itemize}

To give general bounds for these numbers we define a new number:
$$
\alpha = \sum r_i,
$$
the sum taken over all Puiseux branches. This number is bigger than the integrality exponent but not much bigger. And it is clear that $r_i \le \alpha$.

For example, if we have only one branch with seven Puiseux expansions and one characteristic exponent $1/7$, then $\alpha = 1$ and the characteristic exponent is $6/7$.

We have:
\begin{itemize}
\item $r_i \le \alpha$ (clear by definition)
\item $v_i \le \IntExpX \le \alpha$ when there is only one branch, but not in general. Proof: if $F_i = (Y-\gamma_1) \dots (Y-\gamma_r)$ then $(F_i)_Y(\gamma_1) = (1+\gamma_1)(\gamma_2 - \gamma_1)\cdots (\gamma_r - \gamma_1) + 0 + 0 + \dots + 0$. The first factor in the first term has order 0 at $X$, so the order of the product is equal to $\IntExpX$ (it is the same formula).
\end{itemize}
...
\item In that paper the authors provide the bound $\kappa \le \delta / 2$, where $\delta = 2 \IntExpX$, so $\kappa \le \IntExpX$ and it is enough for us to consider $n \ge 2 \IntExpX$.
...
\item I am not sure about this last step. In any case, we can compute the singular part of the Puiseux expansions by truncating at $\alpha + 2 \times \IntExpX$ (a little larger than $3 \times \IntExpX$). The argument is as follows:
    ...
    \begin{enumerate}
    \item We apply Half-RNP algorithm (page 13) to one factor $F_i$.
    ...
    \item So by the algorithm we need $n \ge v_i + r_i$, and $v_i + r_i \le \alpha + \IntExpX$. That is, we need the factors $F_i$ developed to degree $\alpha + \IntExpX$ and by Proposition \ref{prop:factors} we can get this by computing a factorization of $F$ modulo $\alpha + 2 \IntExpX$.
    \end{enumerate}
