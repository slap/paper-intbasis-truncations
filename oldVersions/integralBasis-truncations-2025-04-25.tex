...
\item I am not sure about this last step. In any case, we can compute the singular part of the Puiseux expansions by truncating at $\alpha + 2 \times \IntExpX$ (a little larger than $3 \times \IntExpX$). The argument is as follows:

    \begin{enumerate}
    \item We apply Algorithm \ref{algo:halfRNP} Half-RNP to one factor $F_i$.
    ...
    \item $r_i$ may not be smaller than the integrality exponent but we have $r_i \le \alpha$.
    \item So by Algorithm  \ref{algo:halfRNP}, we need $n \ge v_i + r_i$, and $v_i + r_i \le \alpha + \IntExpX$. That is, we need the factor $F_i$ developed to degree $\alpha + \IntExpX$ and by Proposition \ref{prop:factors} we can get this by computing a factorization of $F$ modulo $\alpha + 2 \IntExpX$.
    \end{enumerate}

\item Putting everything together, we can truncate the powers of $X$ of $F$ at order $\alpha + 2 \IntExpX$ and we will be able to compute the correct factors up to the integrality exponent and the singular part of the Puiseux expansions and hence we will compute the correct integral basis.
